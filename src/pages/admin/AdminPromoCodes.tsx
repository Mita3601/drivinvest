...
const fmtDateTime = (d: string) => new Date(d).toLocaleString("fr-FR", { day: "2-digit", month: "2-digit", hour: "2-digit", minute: "2-digit" });
...
const AdminPromoCodes = () => {
  ...
  const [type, setType] = useState<"balance" | "product_discount" | "deposit_bonus">("balance");
  ...
  const handleCreate = async () => {
    ...
    if (error || !res?.success) {
      toast({ title: "Erreur", description: res?.error || error?.message, variant: "destructive" });
      return;
    }
    toast({ title: "Code créé ✅" });
    setCode(""); setValue(""); setStartsAt(""); setEndsAt(""); setMaxUsers("10");
      ...
      <div className="bg-card border border-border rounded-xl p-4 space-y-3">
        ...
        <div className="grid grid-cols-2 gap-3">
          <div>
            ...
            <Input value={code} onChange={(e) => setCode(e.target.value.toUpperCase())} placeholder="VOGUE2026" />
          </div>
          <div>
            ...
            <Select value={type} onValueChange={(v: any) => setType(v)}>
              ...
              <SelectContent>
                ...
                <SelectItem value="product_discount">Remise produit (%)</SelectItem>
                <SelectItem value="deposit_bonus">Bonus recharge (%)</SelectItem>
              </SelectContent>
            </Select>
          </div>
          <div>
        ...
        <Button onClick={handleCreate} disabled={creating} className="w-full font-bold">
              ...
              return (
                <div key={c.id} className="bg-card border border-border rounded-xl p-3">
                  <div className="flex justify-between items-start">
                    <div>
                      <p className="font-display font-bold tracking-wider">{c.code}</p>
                      <p className="text-xs text-muted-foreground">{typeLabels[c.type]} : <span className="text-foreground font-bold">{c.value}</span></p>
                    </div>
                    <span className={`text-xs px-2 py-1 rounded-md font-bold ${expired || exhausted ? "bg-muted text-muted-foreground" : "bg-success/15 text-success"}`}>
                  ...
                  <div className="grid grid-cols-3 gap-2 text-xs mt-2 text-muted-foreground">
                    ...
                    <div>Utilisé: {c.uses_count}/{c.max_users}</div>
                  </div>
                </div>
              );
            })}
            {(!codes || codes.length === 0) && <p className="text-center text-sm text-muted-foreground py-6">Aucun code</p>}
          </div>
        )}
      </div>
    </div>
