...
const AdminDeposits = () => {
  ...
  const deposits = useMemo(() => (allTx || []).filter((t: any) => t.type === "deposit"), [allTx]);
  const withdrawals = useMemo(() => (allTx || []).filter((t: any) => t.type === "withdrawal"), [allTx]);

  const totalDeposits = deposits.filter((t: any) => t.status === "approved").reduce((s: number, t: any) => s + Number(t.amount), 0);
  const totalWithdrawals = withdrawals.filter((t: any) => t.status === "approved").reduce((s: number, t: any) => s + Number(t.amount), 0);
  const restant = totalDeposits - totalWithdrawals;

  const filtered = filter === "all" ? deposits : deposits.filter((t: any) => t.status === filter);
    ...
    <div className="space-y-4">
      <div className="grid grid-cols-3 gap-2">
        <div className="rounded-xl bg-success/10 border border-success/30 p-3">
          ...
          <p className="font-display font-bold text-success text-sm mt-1">{formatCFA(totalDeposits)} F</p>
        </div>
        <div className="rounded-xl bg-destructive/10 border border-destructive/30 p-3">
          <p className="text-[10px] text-muted-foreground uppercase">Retraits</p>
          <p className="font-display font-bold text-destructive text-sm mt-1">{formatCFA(totalWithdrawals)} F</p>
        </div>
        <div className="rounded-xl bg-primary/10 border border-primary/30 p-3">
          ...
          <p className="font-display font-bold text-primary text-sm mt-1">{formatCFA(restant)} F</p>
        </div>
      </div>
      ...
      {filtered.map((tx: any) => (
        <div key={tx.id} className="rounded-xl bg-secondary border border-border p-4 space-y-2">
          ...
          <div className="text-xs text-muted-foreground space-y-0.5">
            <p>Méthode: {tx.method?.toUpperCase() || "—"} • Expéditeur: {tx.sender_number || "—"}</p>
