...
const AdminWithdrawals = () => {
  ...
  const userIds = useMemo(() => Array.from(new Set((withdrawals || []).map((t: any) => t.user_id))), [withdrawals]);
  ...
  const filtered = (withdrawals || []).filter((t: any) => filter === "all" || t.status === filter);
  ...
  if (isLoading) return <div className="space-y-3">{Array.from({ length: 5 }).map((_, i) => <Skeleton key={i} className="h-32 rounded-xl" />)}</div>;
  ...
  return (
    <div className="space-y-3">
      ...
      <p className="text-muted-foreground text-xs">{filtered.length} retraits</p>
      {filtered.map((tx: any) => {
          ...
          <div key={tx.id} className="rounded-xl bg-secondary border border-border p-4 space-y-2">
            ...
            <div className="grid grid-cols-2 gap-2 text-xs">
              <div className="bg-background/40 rounded-lg p-2">
                ...
                <p className="font-bold text-foreground">{formatCFA(Number(tx.amount))} F</p>
              </div>
              <div className="bg-background/40 rounded-lg p-2">
                <p className="text-muted-foreground text-[10px]">Frais 15%</p>
                <p className="font-bold text-destructive">-{formatCFA(fee)} F</p>
              </div>
              <div className="bg-success/10 rounded-lg p-2 col-span-2">
                <p className="text-muted-foreground text-[10px]">Net à envoyer</p>
                <p className="font-display font-bold text-success">{formatCFA(net)} F</p>
              </div>
            </div>
            <div className="text-xs text-muted-foreground space-y-0.5 pt-1">
              <p><b className="text-foreground">Pays:</b> {tx.country || "—"} • <b className="text-foreground">Réseau:</b> {tx.method?.toUpperCase() || "—"}</p>
              <p><b className="text-foreground">Numéro:</b> {tx.wallet_number || "—"}</p>
              <p>{new Date(tx.created_at).toLocaleString("fr-FR")}</p>
            </div>
            {tx.status === "pending" && (
              <div className="flex gap-2 pt-1">
                <button onClick={() => handleAction(tx.id, "approved")} disabled={processingId === tx.id} className="flex-1 flex items-center justify-center gap-1 bg-success text-success-foreground font-bold py-2 rounded-lg text-xs disabled:opacity-50">
                  <Check className="w-3.5 h-3.5" /> Payé
                </button>
                <button onClick={() => handleAction(tx.id, "rejected")} disabled={processingId === tx.id} className="flex-1 flex items-center justify-center gap-1 bg-destructive text-destructive-foreground font-bold py-2 rounded-lg text-xs disabled:opacity-50">
