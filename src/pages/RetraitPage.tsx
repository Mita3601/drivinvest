  ...
  if (submitted) {
    return (
      <div className="min-h-screen flex flex-col items-center justify-center px-6 text-center gap-4">
        ...
        <h2 className="font-display font-bold text-xl text-foreground">Demande enregistrée</h2>
        <p className="text-muted-foreground text-sm">Votre retrait sera traité sous 24h après validation.</p>
  ...
  return (
    <div className="pb-24 space-y-5">
      <div className="flex items-center gap-3 px-4 pt-4">
        ...
        <h1 className="font-display font-bold text-lg text-foreground">Retrait</h1>
      </div>

      <div className="mx-4 rounded-2xl bg-secondary border border-border p-5 text-center">
        <p className="text-muted-foreground text-xs uppercase tracking-wider font-medium mb-1">Solde retirable</p>
        <p className="font-display font-extrabold text-2xl text-foreground">{formatCFA(balance)} <span className="text-sm text-muted-foreground">FCFA</span></p>
      </div>

      <div className="px-4 space-y-2">
        <label className="text-xs text-muted-foreground uppercase tracking-wider font-medium">Montant à retirer</label>
        <input
          type="number"
          ...
          {operators.map((op) => (
            <button
              ...
              className={`flex-1 py-3 rounded-xl font-bold text-xs border transition-all ${
                operator === op.id ? "bg-primary text-primary-foreground border-primary" : "bg-secondary border-border text-foreground"
              }`}
            >
              {op.label}
            </button>
      ...
      {numAmount > 0 && (
        <div className="mx-4 rounded-2xl bg-secondary border border-border p-4 space-y-2">
          <div className="flex justify-between text-sm">
            ...
            <span className="text-foreground font-semibold">{formatCFA(numAmount)} F</span>
          </div>
          <div className="flex justify-between text-sm">
            <span className="text-muted-foreground">Frais ({feePercent}%)</span>
            <span className="text-destructive font-semibold">-{formatCFA(fee)} F</span>
          </div>
          <div className="border-t border-border pt-2 flex justify-between text-sm">
            <span className="text-foreground font-bold">Vous recevrez</span>
            <span className="text-success font-bold">{formatCFA(netAmount)} F</span>
          </div>
        </div>
      )}
      ...
      <div className="px-4">
        ...
        >
          {loading ? <span className="animate-spin w-5 h-5 border-2 border-primary-foreground border-t-transparent rounded-full" /> : "Demander le retrait"}
        </button>
      </div>
    </div>
  );
