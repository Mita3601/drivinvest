-- Improve daily bonus availability handling (make first claim available if created_at missing)
...
BEGIN
  ...
  IF v_created IS NULL THEN
    -- fallback to allow first claim if created_at missing
    v_created := now() - interval '25 hours';
  END IF;
